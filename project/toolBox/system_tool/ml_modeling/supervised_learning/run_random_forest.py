...
def run_random_forest(
    ...
    通过 Bagging 与特征随机采样构建多棵决策树，降低过拟合风险，提升模型鲁棒性。
    ...
    Examples:
        ...
        >>> result = run_random_forest(df, target_column='target', n_estimators=10)
    ...
    if len(valid_data) < 10:
        raise ValueError("样本数不足，无法训练随机森林模型")

    X = valid_data[feature_columns]
    y = valid_data[target_column]

    n_samples = len(X)
    n_test = max(2, int(n_samples * test_size))
